...
class ModelTrainingV3Tab:
    ...
    def render(self):
        ...
        **核心改進**:
        ...
        with col1:
            ...
            # 進階設定 (不建議修改)
            with st.expander("進階設定 (不建議修改)"):
                st.warning("修改這些設定可能導致模型效果下降")
        ...
        with col1:
            if st.button("開始 V3 訓練", type="primary", use_container_width=True):
    ...
    def _show_latest_report(self):
        ...
        if long_metrics['auc'] > 0.65 and short_metrics['auc'] > 0.65:
            checks.append("[PASS] AUC > 0.65")
        else:
            checks.append("[FAIL] AUC < 0.65 (建議重訓)")
        ...
        if long_max > 0.60 and short_max > 0.60:
            checks.append("[PASS] Max Probability > 0.60")
        elif long_max > 0.50 and short_max > 0.50:
            checks.append("[WARN] Max Probability 0.50-0.60 (可接受)")
        else:
            checks.append("[FAIL] Max Probability < 0.50 (需要調整)")
        
        # 檢查 Precision
        if long_metrics['precision'] > 0.55 and short_metrics['precision'] > 0.55:
            checks.append("[PASS] Precision > 0.55")
        else:
            checks.append("[WARN] Precision < 0.55 (注意)")
    ...
    def _show_v3_models(self):
        ...
        if not v3_long and not v3_short:
            st.info("尚未訓練 V3 模型")
            st.markdown("點擊上方 **'開始 V3 訓練'** 按鈕開始訓練")
            return
