...
class MultiTimeframeFeatureEngineer:
    ...
    def __init__(self):
        logger.info("Initialized MultiTimeframeFeatureEngineer with 2.0 features")
    ...
    def extract_time_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        df['session_overlap'] = ((df['hour'] >= 13) & (df['hour'] < 15)).astype(int)
    ...
    def extract_microstructure_features(self, df_1m: pd.DataFrame) -> pd.DataFrame:
        """第一層: 1分鐘微觀結構特徵 + 2.0 進階特徵"""
        logger.info("Extracting 1m microstructure features with 2.0 enhancements")
        ...
        # === 2.0 進階特徵 ===
        logger.info("Adding Feature Engineering 2.0 - Advanced features")
        
        # 殺手鑇 1: 相對成交量 (RVOL)
        df['volume_ma_240'] = df['volume'].rolling(240).mean()
        df['rvol'] = df['volume'] / df['volume_ma_240']
        df['rvol'] = df['rvol'].replace([np.inf, -np.inf], np.nan).fillna(1.0)
        
        # RVOL 與 Sweep 的交互特徵
        df['sweep_with_high_rvol'] = (
            ((df['1m_bull_sweep'] == 1) | (df['1m_bear_sweep'] == 1)) & 
            (df['rvol'] > 2.5)
        ).astype(int)
        
        logger.info("RVOL feature added")
        
        return df
    
    def extract_tactical_features(self, df_15m: pd.DataFrame) -> pd.DataFrame:
        """第二層: 15分鐘戰術特徵 + 背離特徵"""
        logger.info("Extracting 15m tactical features with divergence")
        ...
        # 記錄前一個 pivot low 的價格與 RSI
        df['prev_pivot_low_price'] = df.loc[df['is_pivot_low'], 'low'].shift(1)
        df['prev_pivot_low_rsi'] = df.loc[df['is_pivot_low'], 'rsi'].shift(1)
        
        df['prev_pivot_low_price'] = df['prev_pivot_low_price'].ffill()
        df['prev_pivot_low_rsi'] = df['prev_pivot_low_rsi'].ffill()
        
        # 底背離: 價格新低但 RSI 未新低
        df['is_bullish_divergence'] = (
            (df['low'] < df['prev_pivot_low_price']) & 
            (df['rsi'] > df['prev_pivot_low_rsi']) &
        ...
        # MACD 背離
        df['prev_pivot_low_macd'] = df.loc[df['is_pivot_low'], 'macd'].shift(1)
        df['prev_pivot_low_macd'] = df['prev_pivot_low_macd'].ffill()
        
        df['is_macd_bullish_divergence'] = (
            (df['low'] < df['prev_pivot_low_price']) & 
            (df['macd'] > df['prev_pivot_low_macd']) &
            (df['is_pivot_low'] == True)
        ).astype(int)
        
        logger.info("Divergence features added (RSI + MACD)")
    ...
    def align_features_to_1m(self, df_1m: pd.DataFrame, df_15m_feat: pd.DataFrame, 
        ...
        # 15m 特徵
        df_15m_shift = df_15m_feat[[
            'open_time', '15m_z_score', '15m_bb_width_pct', 'bb_width',
        ...
        feature_cols = [
            ...
            'bb_width', 'is_bullish_divergence', 'is_macd_bullish_divergence'
        ]
        features_df[feature_cols] = features_df[feature_cols].ffill()
        
        # === 殺手鑇 2: 跨週期波動率壓縮比 ===
        features_df['volatility_squeeze_ratio'] = features_df['bb_width'] / features_df['1d_atr_pct']
        features_df['volatility_squeeze_ratio'] = features_df['volatility_squeeze_ratio'].replace(
            [np.inf, -np.inf], np.nan
        ).fillna(0)
        
        logger.info("Volatility squeeze ratio added")
    ...
    def process_symbol(self, symbol: str) -> pd.DataFrame:
        """處理單一幣種的完整特徵工程流程"""
        logger.info(f"Processing {symbol} with Feature Engineering 2.0")
        ...
        try:
            ...
            if not df_15m_feat.empty and not df_1h_feat.empty and not df_1d_feat.empty:
                ...
                logger.warning(f"Some timeframes missing for {symbol}")
            ...
            logger.info(f"{symbol} processed with 2.0 features: {len(features_df)} records")
            return features_df
